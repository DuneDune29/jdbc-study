...
public class DBFrame extends Frame{
	...
	DBFrame() {
		...
		lid = new Label("아 이 디 : "); 		lpass1 = new Label("비밀번호 : ");
		lpass2 = new Label("비번확인 : "); 	lname = new Label("이   름 : ");
		ljumin = new Label("주민번호 : "); 	ltel = new Label("전화번호 : ");
		laddr = new Label("주   소 : "); 		ljob = new Label("직   업 : ");
		...
		idCheck = new JButton("중복 확인"); 	submit = new JButton("등   록");
		job = new Choice();
		...
		add(ljob); 			add(job);			
		job.add("학 생");			job.add("직장인");			job.add("백 수"); 
		job.add("주 부");			job.add("은행원");			job.add("기 타"); 
